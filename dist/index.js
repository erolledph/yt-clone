...
function isShortDuration(lengthStr) {
    if (!lengthStr)
        return false;
    ...
    for (const item of contents) {
        ...
        if (v) {
            videos.push({
                ...
                channelThumbnail: v.channelThumbnailSupportedRenderers?.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails?.slice(-1)?.[0]?.url || '',
                thumbnail: v.thumbnail?.thumbnails?.slice(-1)?.[0]?.url || '',
                viewCount: v.viewCountText?.simpleText || v.viewCountText?.runs?.map((r) => r.text).join('') || '',
        ...
        if (richVid) {
            videos.push({
                ...
                title: richVid.title?.runs?.map((r) => r.text).join('') || '',
                channelTitle: richVid.ownerText?.runs?.[0]?.text || '',
                channelThumbnail: richVid.channelThumbnailSupportedRenderers?.channelThumbnailWithLinkRenderer?.thumbnail?.thumbnails?.slice(-1)?.[0]?.url || '',
                thumbnail: richVid.thumbnail?.thumbnails?.slice(-1)?.[0]?.url || '',
                viewCount: richVid.viewCountText?.simpleText || richVid.viewCountText?.runs?.map((r) => r.text).join('') || '',
            ...
            if (videoId && videoId.length === 11) {
                ...
                for (const row of metaRows) {
                    for (const part of row.metadataParts || []) {
                        ...
                        if (txt.includes('Playlist') || txt.includes('Updated') || txt.includes('View full'))
                            continue;
                        if (txt.includes('•')) {
                            ...
                            for (const p of parts) {
                                if (p.match(/\d+:\d+/))
                                    length = p;
                                else if (!channel && p && !p.includes('view'))
                                    channel = p;
                            }
                        }
                    }
...
app.get('/api/trending', async (_req, res) => {
    try {
        ...
        const avatarMap = {};
        channelEntries.forEach(([name], i) => { if (avatars[i])
            avatarMap[name] = avatars[i]; });
...
app.get('/api/suggest', async (req, res) => {
    ...
    try {
        const raw = await httpsGet(`https://suggestqueries.google.com/complete/search?client=youtube&ds=yt&gl=PH&q=${encodeURIComponent(q)}`);
        const jsonStr = raw.replace(/^window\.google\.ac\.h\(/, '').replace(/\);?\s*$/, '');
        const parsed = JSON.parse(jsonStr);
        const suggestions = (parsed[1] || []).map((item) => typeof item === 'string' ? item : item[0] || '');
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        const data = await httpsPost(`https://www.youtube.com/youtubei/v1/search?key=${YT_KEY}&prettyPrint=false`, body, YT_HEADERS);
                ...
                for (const item of items) {
                    ...
                    if (v) {
                        ...
                        if (isShortDuration(len)) {
                            videos.push({
                                videoId: v.videoId,
                                title: v.title?.simpleText || v.title?.runs?.map((r) => r.text).join('') || '',
    ...
    try {
        ...
        for (const c of contents) {
            if (c.videoPrimaryInfoRenderer) {
                title = c.videoPrimaryInfoRenderer?.title?.runs?.map((r) => r.text).join('') || '';
            ...
            if (c.videoSecondaryInfoRenderer) {
                ...
                authorName = c.videoSecondaryInfoRenderer?.owner?.videoOwnerRenderer?.title?.runs?.map((r) => r.text).join('') || '';
...
app.get('/api/related/:id', async (req, res) => {
    ...
    try {
        ...
        const data = await httpsPost(`https://www.youtube.com/youtubei/v1/player?key=${YT_KEY}&prettyPrint=false`, body, YT_HEADERS);
        const parsed = JSON.parse(data);
